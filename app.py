...
if predict_clicked:
    ...
    # ── Feature engineering — exactly matching Section 5 of notebook ───────
    ...
    data['util_rate']     = data['BILL_AMT1'] / (data['LIMIT_BAL'] + 1)
    ...
    data['pay_ratio']     = data['PAY_AMT1'] / (data['BILL_AMT1'] + 1)
    ...
    # ── NO log transform — notebook Section 9 confirmed skipping it ────────
    # Skewness was mild (0.9–1.2), log overcorrected, tree models don't need it

    # ── Reorder columns to exactly match saved feature_order ──────────────
    ...
    util_pct       = (bill_vals["BILL_AMT1"] / (LIMIT_BAL + 1)) * 100
    max_delay      = max(pay_vals.values())
    pay_bill_ratio = pamt_vals["PAY_AMT1"] / (bill_vals["BILL_AMT1"] + 1)

    st.markdown("---")
    st.markdown("### 📊 Prediction Result")

    # ── Binary decision — matches dataset: Default (1) / No Default (0) ───
    if prob < 0.50:
        bar_color  = "#3fb950"
